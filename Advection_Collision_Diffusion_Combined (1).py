...
direc='/Users/25016/Desktop/Uniform_long_simu/'
...
#                                    (node.k+n)] #(node.k - self.graph.n) % self.graph.n**2
    ...
    # initialises the node diffusion probabilities and their corresponding indices
...
#                                    (node.k+n)] #(node.k - self.graph.n) % self.graph.n**2
        ...
        # calculate probability of staying
        PE=node.potential_energy() * len(node.particles)
        list_of_PE=[]
        for i in transition_node_indices[1:]:
            node=self.graph.nodes[i]
            list_of_PE.append(node.potential_energy() * len(node.particles))
        upper_value=max(epsilon,PE-min(list_of_PE))
        list_of_PE.append(PE)
        phi_i=upper_value/(max(list_of_PE)-min(list_of_PE))

        p_stay = 1 / (1 + delta * phi_i)
        ...
        # otherwise, calculate "leaving measure" for each neighbor
        for i in range (1, len(transition_node_indices)):
            relative_damage = max(0,list_of_PE[-1] - list_of_PE [i-1] )
            transition_node_probabilities[i] = relative_damage
    ...
    def particle_diffusion_sim(self):
        old_mass=self.graph.get_mass() # keep track of the old mass distribution
        for node in self.graph.nodes:# loop through all nodes
            to_remove=[]                      # keep track of the indices of particles to be removed
            node.time  = delta + node.time
    ...
    def update_PE (self):
        for node in self.graph.nodes:
            PE.loc['node '+str(node.k),'time '+str(int(node.time))] = node.potential_energy()* len(node.particles)
    def simu_plot(self):
...
for k in range (n**2):
    ...
    new_node_vel=np.random.uniform(low=[-5, -5], high=[5, 5], size=(number_of_particles_at_each_node[k],2))
    #np.random.multivariate_normal([0,0],[[1,0],[0,1]],number_of_particles_at_each_node[k])  # initialize a velocity space of the particles at the new node
...
df = pd.DataFrame(np.zeros((len(list_of_IDs),1)) ,index = list_of_IDs, columns=['time '+str(0)])
PE = pd.DataFrame(np.zeros((n**2,1)), index = ['node '+ str(i) for i in range (n**2)], columns=['time '+str(0)])
