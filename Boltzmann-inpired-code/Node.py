...
class Node:
	...
	def ConvectionStep(self, dt):
		...
		for (particle, p) in zip(self.particles, range(len(self.particles))):
	...
	def CollisionStep(self, mu, dt):
		...
		while T+1.0e-10<dt:
			...
			for i in range(1, len(collide), 2):
				...
				w *= self.PostCollisionFunction(self.particles[collide[i-1]].vel, self.particles[collide[i]].vel, w)
	...
	def PostCollisionFunction(self, v, vprime, w):
		we = -w.dot(v-vprime)
